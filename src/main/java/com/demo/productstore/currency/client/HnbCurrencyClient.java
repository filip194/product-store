...
public class HnbCurrencyClient implements CurrencyClient {
    ...
    public Double getMidMarketExchangeRate(CurrencyCountryCode currencyCountryCode) {
        ...
        if (response.getStatusCode().is2xxSuccessful() &&
                ...
                Objects.nonNull(response.getBody().getMidRate())) {
            try {
                return Double.valueOf(response.getBody().getMidRate());
            } catch (NumberFormatException ex) {
                throw new IllegalStateException("HNB API returned invalid mid market exchange rate value: " + response.getBody().getMidRate());
            }
        }
